...
export default {
  ...
  computed: {
    previewSourceCode() {
      return this.sourceCode.replace(/'\.\.\/\.\.\/index'/g, `'@tencent/du_ui'`);
    },
  },
  async mounted() {
    if (this.compName && this.demoName) {
      if (isDev) {
        ...
        this.sourceCode = await fetch(`${isDev ? '' : '/du_ui'}/packages/${this.compName}/docs/${this.demoName}.vue`).then((res) => res.text());
      }
    }
    await this.$nextTick();
    Prism.highlightAll();
